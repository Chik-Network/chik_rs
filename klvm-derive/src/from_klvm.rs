...
pub fn from_klvm(ast: DeriveInput) -> TokenStream {
    ...
    match &ast.data {
        ...
        Data::Union(_union) => panic!("cannot derive `FromKlvm` for a union"),
    }
}
...
fn impl_for_struct(
    ...
    let body = quote! {
        let #destructure_macro!( #( #field_names, )* ) =
            ...
            as #crate_name::FromKlvm<#node_name>>::from_klvm(decoder, node)?;
        Ok(Self #initializer)
    };

    generate_from_klvm(crate_name, ast, &node_name, &body)
}
...
fn impl_for_enum(
            ...
            if initializer.is_empty() {
                ...
                quote! {
                    #discriminant_ident => {
                        let #destructure_macro!( #( #field_names ),* ) =
                            <#match_macro!( #( #field_types ),* )
                            as #crate_name::FromKlvm<#node_name>>::from_klvm(decoder, args.0)?;
    ...
    let parse_value = if has_initializers {
        quote! {
            let (value, args) = <(#int_repr, #crate_name::Raw<#node_name>)>::from_klvm(decoder, node)?;
        }
    } else {
        quote! {
            let value = #int_repr::from_klvm(decoder, node)?;
        }
    };
        ...
        .map(|variant_info| {
            ...
            quote! {
                if let Ok(#destructure_macro!( #( #field_names ),* )) =
                    <#match_macro!( #( #field_types ),* )
                    as #crate_name::FromKlvm<#node_name>>::from_klvm(decoder, decoder.clone_node(&node))
                {
...
fn generate_from_klvm(
    ...
    add_trait_bounds(
        &mut ast.generics,
        parse_quote!(#crate_name::FromKlvm<#node_name>),
    );
    ...
    quote! {
        ...
        impl #impl_generics #crate_name::FromKlvm<#node_name>
        for #type_name #ty_generics #where_clause {
            fn from_klvm(
                decoder: &impl #crate_name::KlvmDecoder<Node = #node_name>,
                node: #node_name,
