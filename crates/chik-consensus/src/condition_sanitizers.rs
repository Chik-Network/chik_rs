...
pub fn sanitize_message_mode(a: &Allocator, node: NodePtr) -> Result<u32, ValidationErr> {
    let Some(mode) = a.small_number(node) else {
        return Err(ValidationErr::Err(ErrorCode::InvalidMessageMode));
    };
...
fn test_sanitize_mode(#[case] value: i64, #[case] pass: bool) {
    ...
    if pass {
        ...
        assert_eq!(ret.unwrap_err().error_code(), ErrorCode::InvalidMessageMode);
    }
}
...
fn test_sanitize_amount() {
    ...
    assert_eq!(
        amount_tester(&[0x80]).unwrap_err().error_code(),
        ErrorCode::InvalidCoinAmount
    );
    assert_eq!(
        amount_tester(&[0xff]).unwrap_err().error_code(),
        ErrorCode::InvalidCoinAmount
    );
    assert_eq!(
        amount_tester(&[0xff, 0]).unwrap_err().error_code(),
        ErrorCode::InvalidCoinAmount
    );
    ...
    assert_eq!(
        amount_tester(&[0, 0, 0, 0xff]).unwrap_err().error_code(),
        ErrorCode::InvalidCoinAmount
    );
    assert_eq!(
        amount_tester(&[0, 0, 0, 0x80]).unwrap_err().error_code(),
        ErrorCode::InvalidCoinAmount
    );
    assert_eq!(
        amount_tester(&[0, 0, 0, 0x7f]).unwrap_err().error_code(),
        ErrorCode::InvalidCoinAmount
    );
    assert_eq!(
        amount_tester(&[0, 0, 0]).unwrap_err().error_code(),
        ErrorCode::InvalidCoinAmount
    );
