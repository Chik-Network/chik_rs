...
fn hash_atom_list(
    ...
    while count > 0 {
        let Some((arg, next)) = a.next(args) else {
            return Err(ValidationErr::Err(ErrorCode::InvalidCondition));
        };
        args = next;
        count -= 1;
...
/// across different chik_rs versions.
...
mod tests {
    ...
    fn test_hash_atom_list_not_enough_items() {
        ...
        assert_eq!(
            hash_atom_list(&mut ctx1, &a, list, 2)
                .unwrap_err()
                .error_code(),
    ...
    fn test_hash_atom_list_pair() {
        ...
        assert_eq!(
            hash_atom_list(&mut ctx1, &a, list, 1)
                .unwrap_err()
                .error_code(),
            ErrorCode::InvalidCondition
        );
