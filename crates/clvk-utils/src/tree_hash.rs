...
use clvkr::allocator::{Allocator, NodePtr, NodeVisitor, ObjectType, SExp};
...
fn test_tree_hash_from_bytes() {
    use clvkr::serde::{node_to_bytes, node_to_bytes_backrefs};
    ...
    let serialized_clvk = node_to_bytes(&a, root).expect("node_to_bytes");
    let serialized_clvk_backrefs =
        node_to_bytes_backrefs(&a, root).expect("node_to_bytes_backrefs");

    let hash1 = tree_hash_from_bytes(&serialized_clvk).expect("tree_hash_from_bytes");
    let hash2 = tree_hash_from_bytes(&serialized_clvk_backrefs).expect("tree_hash_from_bytes");
    let hash3 = tree_hash(&a, root);

    assert!(serialized_clvk.len() > serialized_clvk_backrefs.len());
...
fn test_tree_hash_cached(
    ...
    use clvkr::serde::{node_from_bytes_backrefs, node_to_bytes_backrefs};
