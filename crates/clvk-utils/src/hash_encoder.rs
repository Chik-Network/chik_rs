...
use clvk_traits::{ClvkEncoder, ToClvk, ToClvkError};
use clvkr::Atom;
...
{
    fn tree_hash(&self) -> TreeHash {
        self.to_clvk(&mut TreeHasher).unwrap()
    }
}
...
impl ClvkEncoder for TreeHasher {
    type Node = TreeHash;

    fn encode_atom(&mut self, bytes: Atom<'_>) -> Result<Self::Node, ToClvkError> {
...
impl ToClvk<TreeHasher> for TreeHash {
    fn to_clvk(&self, _encoder: &mut TreeHasher) -> Result<TreeHash, ToClvkError> {
        Ok(*self)
    }
}
...
mod tests {
    ...
    fn test_tree_hash() {
        ...
        // Its tree hash is known, and its CLVK is `(=)`.
        assert_eq!(
