...
#[cfg(test)]
mod tests {
    use clvk_traits::{ToClvk, clvk_curried_args};
    use clvkr::Allocator;
    use hex::ToHex;
    ...
    fn test_equivalence() {
        ...
        let args = clvk_curried_args!(5, 8).to_clvk(&mut a).unwrap();
        let curried = CurriedProgram { program, args }.to_clvk(&mut a).unwrap();
