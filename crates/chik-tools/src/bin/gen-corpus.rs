// This is a throw-away tool to scan a chik blockchain database for spends that
...
/// Analyze the spends in a chik blockchain database
#[derive(Parser, Debug)]
        ...
        |height, block, block_refs| {
            ...
            pool.execute(move || {
                ...
                if args.block_generators {
                    let directory = "../chik-protocol/fuzz/corpus/run-generator";
                    let _ = std::fs::create_dir_all(directory);
                    write(format!("{directory}/{height}.bundle"), generator).expect("write");

                    let directory = "../chik-protocol/fuzz/corpus/additions-and-removals";
                    let _ = std::fs::create_dir_all(directory);
                    write(format!("{directory}/{height}.bundle"), generator).expect("write");
                ...
                if args.puzzles || args.spend_bundles || args.coin_spends {
                    visit_spends(
                        ...
                        |a, parent_coin_info, amount, puzzle, solution| {
                            ...
                            if run_puzzle {
                                let directory = "../chik-consensus/fuzz/corpus/run-puzzle";
                                let _ = std::fs::create_dir_all(directory);
                            ...
                            if fast_forward {
                                let directory = "../chik-consensus/fuzz/corpus/fast-forward";
                                let _ = std::fs::create_dir_all(directory);
                ...
                if args.coin_spends && !bundle.coin_spends.is_empty() {
                    let directory = "../chik-consensus/fuzz/corpus/solution-generator";
                    let _ = std::fs::create_dir_all(directory);
                    let mut f =
