...
#[cfg_attr(feature = "py-bindings", derive(chik_datalayer_macro::PythonError))]
#[derive(Debug, Error)]
pub enum Error {
    #[error("IO error: {0}")]
    Io(#[from] std::io::Error),
    #[error("failed loading metadata: {0}")]
    FailedLoadingMetadata(chik_traits::chik_error::Error),
    #[error("failed loading node: {0}")]
    FailedLoadingNode(chik_traits::chik_error::Error),
    ...
    Streaming(chik_traits::chik_error::Error),
