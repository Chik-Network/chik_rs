...
pub fn from_klvm(ast: DeriveInput) -> TokenStream {
    let parsed = parse("FromKlvm", &ast);
...
fn field_parser_fn_body(
    ...
    for (i, field) in fields.iter().enumerate() {
        ...
        let mut decoded_value = quote! {
            <#ty as #crate_name::FromKlvm<#decoder_name>>::from_klvm(decoder, #ident)
        };
        ...
        if let Some(value) = &field.constant {
            ...
            body.extend(quote! {
                ...
                if #decoded_value != value {
                    return Err(#crate_name::FromKlvmError::Custom(
                        format!(
        ...
        Repr::List => {
            ...
            quote! {
                ...
                if !atom_ref.is_empty() {
                    return Err(#crate_name::FromKlvmError::WrongAtomLength {
                        expected: 0,
                        found: atom_ref.len(),
                    });
                }
...
fn impl_for_enum(
    ...
    if enum_info.is_untagged {
        ...
        body.extend(quote! {
            Err(#crate_name::FromKlvmError::Custom(
        ...
        if enum_info.default_repr == Repr::Atom {
            ...
            body.extend(quote! {
                let discriminant = <#discriminant_type as #crate_name::FromKlvm<#decoder_name>>::from_klvm(
                    decoder,
                    node,
                )?;
                ...
                match discriminant {
                    ...
                    _ => Err(#crate_name::FromKlvmError::Custom(
...
fn enum_variant_parsers(
    ...
    for variant in &enum_info.variants {
        ...
        variant_parsers.push(quote! {
            |mut node: #node_name| -> ::std::result::Result<Self, #crate_name::FromKlvmError> {
                #body
            }
        });
    }
...
// This generates the `FromKlvm` trait implementation and augments generics with the `FromKlvm` bound.
fn trait_impl(
    mut ast: DeriveInput,
    crate_name: &Ident,
    node_name: &Ident,
    ...
    // Every generic type must implement `FromKlvm` as well in order for the derived type to implement `FromKlvm`.
    // This isn't always perfect, but it's how derive macros work.
    add_trait_bounds(
        &mut ast.generics,
        &parse_quote!(#crate_name::FromKlvm<#decoder_name>),
    );
    ...
    ast.generics.params.push(GenericParam::Type(
        parse_quote!(#decoder_name: #crate_name::KlvmDecoder<Node = #node_name>),
    ));
    ...
    quote! {
        ...
        impl #impl_generics #crate_name::FromKlvm<#decoder_name>
        for #type_name #ty_generics #where_clause {
            fn from_klvm(
                decoder: &#decoder_name,
                mut node: #node_name,
            ) -> ::std::result::Result<Self, #crate_name::FromKlvmError> {
                #body
            }
        }
    }
