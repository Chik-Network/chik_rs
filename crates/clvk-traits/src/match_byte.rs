...
impl<N, E: ClvkEncoder<Node = N>, const BYTE: u8> ToClvk<E> for MatchByte<BYTE> {
...
impl<N, D: ClvkDecoder<Node = N>, const BYTE: u8> FromClvk<D> for MatchByte<BYTE> {
    fn from_clvk(decoder: &D, node: N) -> Result<Self, FromClvkError> {
        match decoder.decode_atom(&node)?.as_ref() {
            ...
            _ => Err(FromClvkError::Custom(format!(
...
mod tests {
    ...
    fn test_byte<const BYTE: u8>() {
        ...
        let encoded = match_byte.to_clvk(&mut allocator).unwrap();
        MatchByte::<BYTE>::from_clvk(&allocator, encoded).unwrap();
    }
