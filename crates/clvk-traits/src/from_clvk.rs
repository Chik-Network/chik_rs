...
use crate::{ClvkDecoder, FromClvkError, decode_number};

pub trait FromClvk<D>: Sized
where
    D: ClvkDecoder,
{
    fn from_clvk(decoder: &D, node: D::Node) -> Result<Self, FromClvkError>;
}

macro_rules! clvk_primitive {
    ($primitive:ty, $signed:expr) => {
        impl<N, D: ClvkDecoder<Node = N>> FromClvk<D> for $primitive {
...
clvk_primitive!(u8, false);
clvk_primitive!(i8, true);
clvk_primitive!(u16, false);
clvk_primitive!(i16, true);
clvk_primitive!(u32, false);
clvk_primitive!(i32, true);
clvk_primitive!(u64, false);
clvk_primitive!(i64, true);
clvk_primitive!(u128, false);
clvk_primitive!(i128, true);
clvk_primitive!(usize, false);
clvk_primitive!(isize, true);

impl<N, D: ClvkDecoder<Node = N>> FromClvk<D> for BigInt {
    fn from_clvk(decoder: &D, node: N) -> Result<Self, FromClvkError> {
        decoder.decode_bigint(&node)
    }
}

impl<N, D: ClvkDecoder<Node = N>> FromClvk<D> for bool {
    fn from_clvk(decoder: &D, node: N) -> Result<Self, FromClvkError> {
        let atom = decoder.decode_atom(&node)?;
        match atom.as_ref() {
            [] => Ok(false),
            [1] => Ok(true),
            _ => Err(FromClvkError::Custom(
...
impl<N, D: ClvkDecoder<Node = N>, T> FromClvk<D> for Box<T>
where
    T: FromClvk<D>,
{
    fn from_clvk(decoder: &D, node: N) -> Result<Self, FromClvkError> {
        T::from_clvk(decoder, node).map(Box::new)
    }
}

impl<N, D: ClvkDecoder<Node = N>, T> FromClvk<D> for Rc<T>
where
    T: FromClvk<D>,
{
    fn from_clvk(decoder: &D, node: N) -> Result<Self, FromClvkError> {
        T::from_clvk(decoder, node).map(Rc::new)
    }
}

impl<N, D: ClvkDecoder<Node = N>, T> FromClvk<D> for Arc<T>
where
    T: FromClvk<D>,
{
    fn from_clvk(decoder: &D, node: N) -> Result<Self, FromClvkError> {
        T::from_clvk(decoder, node).map(Arc::new)
    }
}

impl<N, D: ClvkDecoder<Node = N>, A, B> FromClvk<D> for (A, B)
where
    A: FromClvk<D>,
    B: FromClvk<D>,
{
    fn from_clvk(decoder: &D, node: N) -> Result<Self, FromClvkError> {
        ...
        let first = A::from_clvk(decoder, first)?;
        let rest = B::from_clvk(decoder, rest)?;
        Ok((first, rest))
    }
}

impl<N, D: ClvkDecoder<Node = N>> FromClvk<D> for () {
    fn from_clvk(decoder: &D, node: N) -> Result<Self, FromClvkError> {
        let bytes = decoder.decode_atom(&node)?;
        if bytes.as_ref().is_empty() {
            Ok(())
        } else {
            Err(FromClvkError::WrongAtomLength {
                expected: 0,
...
impl<N, D: ClvkDecoder<Node = N>, T, const LEN: usize> FromClvk<D> for [T; LEN]
where
    T: FromClvk<D>,
{
    fn from_clvk(decoder: &D, mut node: N) -> Result<Self, FromClvkError> {
        let mut items = Vec::with_capacity(LEN);
        loop {
            if let Ok((first, rest)) = decoder.decode_pair(&node) {
                if items.len() >= LEN {
                    return Err(FromClvkError::ExpectedAtom);
                }

                items.push(T::from_clvk(decoder, first)?);
                node = rest;
            } else {
                let bytes = decoder.decode_atom(&node)?;
                if bytes.as_ref().is_empty() {
                    return items.try_into().or(Err(FromClvkError::ExpectedPair));
                }
...
impl<N, D: ClvkDecoder<Node = N>, T> FromClvk<D> for Vec<T>
where
    T: FromClvk<D>,
{
    fn from_clvk(decoder: &D, mut node: N) -> Result<Self, FromClvkError> {
...
impl<N, D: ClvkDecoder<Node = N>, T> FromClvk<D> for Option<T>
where
    T: FromClvk<D>,
{
    fn from_clvk(decoder: &D, node: N) -> Result<Self, FromClvkError> {
        ...
        Ok(Some(T::from_clvk(decoder, node)?))
    }
}

impl<N, D: ClvkDecoder<Node = N>> FromClvk<D> for String {
    fn from_clvk(decoder: &D, node: N) -> Result<Self, FromClvkError> {
        let bytes = decoder.decode_atom(&node)?;
...
impl<N, D: ClvkDecoder<Node = N>> FromClvk<D> for chik_bls::PublicKey {
    fn from_clvk(decoder: &D, node: N) -> Result<Self, FromClvkError> {
        let bytes = decoder.decode_atom(&node)?;
        let error = Err(FromClvkError::WrongAtomLength {
            expected: 48,
            found: bytes.as_ref().len(),
        });
        let bytes: [u8; 48] = bytes.as_ref().try_into().or(error)?;
        Self::from_bytes(&bytes).map_err(|error| FromClvkError::Custom(error.to_string()))
    }
}

#[cfg(feature = "chik-bls")]
impl<N, D: ClvkDecoder<Node = N>> FromClvk<D> for chik_bls::Signature {
    fn from_clvk(decoder: &D, node: N) -> Result<Self, FromClvkError> {
        let bytes = decoder.decode_atom(&node)?;
        let error = Err(FromClvkError::WrongAtomLength {
            expected: 96,
...
impl<D> FromClvk<D> for chik_secp::K1PublicKey
where
    D: ClvkDecoder,
{
    fn from_clvk(decoder: &D, node: D::Node) -> Result<Self, FromClvkError> {
        ...
        let bytes: [u8; Self::SIZE] =
            atom.as_ref()
                .try_into()
                .map_err(|_| FromClvkError::WrongAtomLength {
                    expected: Self::SIZE,
                    found: atom.len(),
                })?;
        Self::from_bytes(&bytes).map_err(|error| FromClvkError::Custom(error.to_string()))
...
#[cfg(feature = "chik-secp")]
impl<D> FromClvk<D> for chik_secp::K1Signature
where
    D: ClvkDecoder,
{
    fn from_clvk(decoder: &D, node: D::Node) -> Result<Self, FromClvkError> {
...
#[cfg(feature = "chik-secp")]
impl<D> FromClvk<D> for chik_secp::R1PublicKey
where
    D: ClvkDecoder,
{
    fn from_clvk(decoder: &D, node: D::Node) -> Result<Self, FromClvkError> {
...
#[cfg(feature = "chik-secp")]
impl<D> FromClvk<D> for chik_secp::R1Signature
where
    D: ClvkDecoder,
{
    fn from_clvk(decoder: &D, node: D::Node) -> Result<Self, FromClvkError> {
...
mod tests {
    ...
    fn decode<T>(a: &mut Allocator, hex: &str) -> Result<T, FromClvkError>
    where
        T: FromClvk<Allocator>,
    {
        let bytes = hex::decode(hex).unwrap();
        let actual = node_from_bytes(a, &bytes).unwrap();
        T::from_clvk(a, actual)
    }
