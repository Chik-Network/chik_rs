...
for implementing the `ToClvk` and `FromClvk` traits on structs and enums.
...
These are referred to as representations and are specified using the `#[clvk(...)]` attribute.
...
This is how non-atomic values are structured in CLVK.
...
```rust
use clvkr::Allocator;
use clvk_traits::{ToClvk, FromClvk};

#[derive(Debug, PartialEq, Eq, ToClvk, FromClvk)]
#[clvk(proper_list)]
struct Tiers {
    high: u8,
    medium: u8,
...
// The CLVK representation for this is `(10 5 1)`.
...
assert_eq!(Tiers::from_clvk(a, ptr).unwrap(), value);
```

### List

The list representation is the same as proper list, except it does not check the nil terminator when parsing.
This allows it to be lenient to additional parameters that are in the CLVK object, which is especially useful
...
This represents the argument part of a curried CLVK program.
In Chik, currying commits to and partially applies some of the arguments of a program, without calling it.

The arguments are quoted and terminated with `1`, which is how partial application is implemented in CLVK.
...
// The CLVK representation for this is `(c (q . 4328714) (c (q . 5) 1))`.
let args = PuzzleArgs {
...
let a = &mut Allocator::new();
let ptr = args.to_clvk(a).unwrap();
assert_eq!(PuzzleArgs::from_clvk(a, ptr).unwrap(), args);
```

### Transparent

If you want a struct to have the same CLVK representation as its inner struct (a newtype), you can use the `transparent` representation.
...
// The CLVK representation for this is the same as the string itself.
// So `"Hello"` in this case.
let string = CustomString("Hello".to_string());

let a = &mut Allocator::new();
let ptr = string.to_clvk(a).unwrap();
assert_eq!(CustomString::from_clvk(a, ptr).unwrap(), string);
```
...
You can set a field as optional by marking it as `#[clvk(default)]`.
...
// The CLVK representation of this is `("Bob" "bob@example.com")`.
...
let a = &mut Allocator::new();
let ptr = person.to_clvk(a).unwrap();
assert_eq!(Person::from_clvk(a, ptr).unwrap(), person);
```
...
// The CLVK representation for this is `("Bob" 24)`.
...
You can consume the rest of the list items (or curried arguments, if using the `curry` representation) by using `#[clvk(rest)]`.
...
// The CLVK representation of this is `(5 . 2)` (with no nil terminator).
let point = Point {
    x: 5,
    y: 2,
};
...
let a = &mut Allocator::new();
let ptr = point.to_clvk(a).unwrap();
assert_eq!(Point::from_clvk(a, ptr).unwrap(), point);
```
...
// The CLVK representation of this is `("First Item" 1 2 3 4 5)`.
...
let a = &mut Allocator::new();
let ptr = items.to_clvk(a).unwrap();

// We parse `("First Item" . <rest>)`
let items = Items::<NodePtr>::from_clvk(a, ptr).unwrap();
assert_eq!(items.first, "First Item".to_string());

// Then parse rest into a separate list of `(1 2 3 4 5)`.
let rest: [u8; 5] = FromClvk::from_clvk(a, items.rest).unwrap();
...
In most cases, the CLVK representation of the enum will need to contain this discriminant as the first argument.
For convenience, this is the behavior when deriving `ToClvk` and `FromClvk` for enums by default.
...
// The CLVK representation of this is just `0`.
let status = Status::Pending;

let a = &mut Allocator::new();
let ptr = status.to_clvk(a).unwrap();
assert_eq!(Status::from_clvk(a, ptr).unwrap(), status);
```
...
// The CLVK representation of this is `36`.
let status = Status::Pending;
...
// The CLVK representation of this is `(42)`.
let mode = SpendMode::AppendValue {
    value: 42
};

let a = &mut Allocator::new();
let ptr = mode.to_clvk(a).unwrap();
assert_eq!(SpendMode::from_clvk(a, ptr).unwrap(), mode);
```
...
This is what `#[clvk(untagged)]` allows you to do.
...
// The CLVK representation of this is `((42 42 42 42))`.
let value = Either::ShortList([42; 4]);

let a = &mut Allocator::new();
let ptr = value.to_clvk(a).unwrap();
assert_eq!(Either::from_clvk(a, ptr).unwrap(), value);
```

## Constant Values

Sometimes you may want to include constants inside of a struct without actually exposing them as fields.
It's possible to do this with `#[clvk(constant = ...)]`, however you must use an attribute macro to remove the constant fields.

This has to be done in the proper order, or it will not work.

The order is as follows:

- Derive `ToClvk` and `FromClvk`, so that the constant fields are serialized and deserialized properly.
...
- Write any `#[clvk(...)]` options you want to use.

Here is an example of this:

```rust
use clvkr::Allocator;
use clvk_traits::{apply_constants, ToClvk, FromClvk};
...
struct CustomTerminator {
    value: u32,
    #[clvk(constant = 42, rest)]
    terminator: u8,
}

// The CLVK representation of this is `(100 . 42)`.
...
let a = &mut Allocator::new();
let ptr = value.to_clvk(a).unwrap();
assert_eq!(CustomTerminator::from_clvk(a, ptr).unwrap(), value);
```

## Crate Name

You can override the name of the `clvk_traits` crate used within the macros:

```rust
use clvkr::Allocator;
use clvk_traits::{self as renamed_clvk_traits, ToClvk, FromClvk};

#[derive(Debug, PartialEq, Eq, ToClvk, FromClvk)]
#[clvk(list, crate_name = renamed_clvk_traits)]
struct Example;
```
