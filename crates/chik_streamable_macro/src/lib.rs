...
pub fn streamable(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let attrs = if matches!(found_crate, FoundCrate::Itself) {
        ...
        let json_dict = if no_json {
            ...
            quote! {
                #[cfg_attr(feature = "py-bindings", derive(chik_py_streamable_macro::PyJsonDict))]
            }
        };
...
pub fn chik_streamable_macro(input: TokenStream) -> TokenStream {
        ...
        Data::Enum(e) => {
            ...
            let ret = quote! {
                impl #crate_name::Streamable for #ident {
                    fn update_digest(&self, digest: &mut chik_sha2::Sha256) {
    ...
    if !fnames.is_empty() {
        ...
        let ret = quote! {
            impl #crate_name::Streamable for #ident {
                fn update_digest(&self, _digest: &mut chik_sha2::Sha256) {}
