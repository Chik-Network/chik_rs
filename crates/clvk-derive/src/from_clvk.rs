...
pub fn from_clvk(ast: DeriveInput) -> TokenStream {
    let parsed = parse("FromClvk", &ast);
...
fn field_parser_fn_body(
    ...
    for (i, field) in fields.iter().enumerate() {
        ...
        let mut decoded_value = quote! {
            <#ty as #crate_name::FromClvk<#decoder_name>>::from_clvk(decoder, #ident)
        };
        ...
        if let Some(value) = &field.constant {
            ...
            body.extend(quote! {
                ...
                if #decoded_value != value {
                    return Err(#crate_name::FromClvkError::Custom(
                        format!(
        ...
        Repr::ProperList => {
            ...
            quote! {
                ...
                if !atom_ref.is_empty() {
                    return Err(#crate_name::FromClvkError::WrongAtomLength {
                        expected: 0,
                        found: atom_ref.len(),
                    });
                }
...
fn impl_for_enum(
    ...
    if enum_info.is_untagged {
        ...
        body.extend(quote! {
            Err(#crate_name::FromClvkError::Custom(
        ...
        if enum_info.default_repr == Repr::Atom {
            ...
            body.extend(quote! {
                let discriminant = <#discriminant_type as #crate_name::FromClvk<#decoder_name>>::from_clvk(
                    decoder,
                    node,
                )?;
                ...
                match discriminant {
                    ...
                    _ => Err(#crate_name::FromClvkError::Custom(
...
fn enum_variant_parsers(
    ...
    for variant in &enum_info.variants {
        ...
        variant_parsers.push(quote! {
            |mut node: #node_name| -> ::std::result::Result<Self, #crate_name::FromClvkError> {
                #body
            }
        });
    }
...
// This generates the `FromClvk` trait implementation and augments generics with the `FromClvk` bound.
fn trait_impl(
    mut ast: DeriveInput,
    crate_name: &Ident,
    node_name: &Ident,
    ...
    // Every generic type must implement `FromClvk` as well in order for the derived type to implement `FromClvk`.
    // This isn't always perfect, but it's how derive macros work.
    add_trait_bounds(
        &mut ast.generics,
        &parse_quote!(#crate_name::FromClvk<#decoder_name>),
    );
    ...
    ast.generics.params.push(GenericParam::Type(
        parse_quote!(#decoder_name: #crate_name::ClvkDecoder<Node = #node_name>),
    ));
    ...
    quote! {
        ...
        impl #impl_generics #crate_name::FromClvk<#decoder_name>
        for #type_name #ty_generics #where_clause {
            fn from_clvk(
                decoder: &#decoder_name,
                mut node: #node_name,
            ) -> ::std::result::Result<Self, #crate_name::FromClvkError> {
                #body
            }
        }
    }
