...
/// The representation of fields when converted to and from CLVK.
...
impl Repr {
    pub fn expect(repr: Option<Repr>) -> Repr {
        repr.expect(
            "missing either `list`, `proper_list`, `curry`, `transparent`, or `atom` in `clvk` attribute options",
        )
    }
}
...
/// All of the possible options of the `clvk` attribute and the enum `repr` attribute.
/// They must be validated after being parsed to prevent invalid option configurations.
pub struct ClvkOptions {
    ...
    /// The name of the `clvk_traits` crate to use, useful for renamed dependencies for example.
    pub crate_name: Option<Ident>,
    /// The default value of the field, if it's not present in the CLVK object.
    ...
    /// Whether the field is a rest field, which will consume the rest of the CLVK object.
    pub rest: bool,
}

/// All of the possible options of the `clvk` attribute.
enum ClvkOption {
    Repr(Repr),
    Constant(Expr),
    CrateName(Ident),
...
/// Parses the `clvk` attribute options and `repr` option from the given attributes.
pub fn parse_clvk_options(attrs: &[Attribute]) -> ClvkOptions {
    let mut options = ClvkOptions {
        repr: None,
        constant: None,
        untagged: false,
        enum_repr: None,
    ...
    for attr in attrs {
        ...
        if ident == "repr" {
            ...
            // For example, `#[repr(C)]` should not be relevant to the CLVK conversions.
        ...
        let parsed_options = attr
            .parse_args_with(Punctuated::<ClvkOption, Token![,]>::parse_terminated)
            .unwrap_or_else(|error| panic!("failed to parse `clvk` attribute options: {error}"));
