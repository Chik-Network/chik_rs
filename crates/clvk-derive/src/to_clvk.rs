...
pub fn to_clvk(ast: DeriveInput) -> TokenStream {
    let parsed = parse("ToClvk", &ast);
...
fn encode_fields(
    ...
    for (i, field) in fields.iter().enumerate().rev() {
        ...
        if_body.extend(quote! {
            let value_node = <#ty as #crate_name::ToClvk<#encoder_name>>::to_clvk(&#value_name, encoder)?;
        });
    ...
    let body = if enum_info.is_untagged {
        ...
        if enum_info.default_repr == Repr::Atom {
            ...
            quote! {
                ...
                match self {
                    #( Self::#variant_names => {
                        <#discriminant_type as #crate_name::ToClvk<#encoder_name>>::to_clvk(
                            &#discriminant_names,
                            encoder,
                        )
                    }, )*
            ...
            quote! {
                ...
                match self {
                    #( #variant_destructures => {
                        ...
                        let discriminant_node = <#discriminant_type as #crate_name::ToClvk<#encoder_name>>::to_clvk(
                            &#discriminant_names,
                            encoder,
                        )?;
...
fn trait_impl(
    ...
    // Every generic type must implement `ToClvk` as well in order for the derived type to implement `ToClvk`.
    // This isn't always perfect, but it's how derive macros work.
    add_trait_bounds(
        &mut ast.generics,
        &parse_quote!(#crate_name::ToClvk<#encoder_name>),
    );
    ...
    ast.generics.params.push(GenericParam::Type(
        parse_quote!(#encoder_name: #crate_name::ClvkEncoder<Node = #node_name>),
    ));
    ...
    quote! {
        ...
        impl #impl_generics #crate_name::ToClvk<#encoder_name>
        for #type_name #ty_generics #where_clause {
            fn to_clvk(
                &self,
                encoder: &mut #encoder_name
            ) -> ::std::result::Result<#node_name, #crate_name::ToClvkError> {
                #body
            }
        }
    }
