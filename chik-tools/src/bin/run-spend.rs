use chik::gen::conditions::Condition;
use chik_protocol::Bytes32;
use chik_traits::Streamable;
use clap::Parser;
use hex_literal::hex;
use klvm_traits::{FromKlvm, ToKlvm, ToNodePtr};
...
fn print_puzzle_info(a: &Allocator, puzzle: NodePtr, solution: NodePtr) {
    ...
    match tree_hash(a, uncurried.program) {
        SINGLETON_MOD_HASH => {
            ...
            let inner_solution = if let Ok(sol) = SingletonSolution::from_klvm(a, solution) {
                println!("  solution");
                println!("    lineage-proof: {:?}", sol.lineage_proof);
                println!("    amount: {}", sol.amount);
                sol.inner_solution
            } else if let Ok(sol) = EveSingletonSolution::from_klvm(a, solution) {
...
fn main() {
    use chik::gen::conditions::parse_args;
    use chik::gen::flags::ENABLE_SOFTFORK_CONDITION;
    use chik::gen::opcodes::parse_opcode;
    use chik::gen::validation_error::{first, rest};
