...
fn compute_plot_id_v1(
    ...
    chik_protocol::compute_plot_id_v1(&plot_pk, pool_pk.as_ref(), pool_contract.as_ref())
}
...
fn compute_plot_id_v2(
    ...
    chik_protocol::compute_plot_id_v2(
        strength,
        &plot_pk,
        pool_pk.as_ref(),
...
#[pyclass]
pub struct Prover(chik_pos2::Prover);
...
impl Prover {
    ...
    pub fn from_bytes(b: &[u8]) -> PyResult<Self> {
        Ok(Self(bincode::deserialize::<chik_pos2::Prover>(b).map_err(
...
pub fn quality_string_from_proof(
    ...
    chik_pos2::quality_string_from_proof(&plot_id.to_bytes(), size, plot_strength, proof).map(
        |quality| -> Bytes32 {
            let mut sha256 = Sha256::new();
            sha256.update(chik_pos2::serialize_quality(
                &quality.chain_links,
...
#[pymodule]
pub fn chik_rs(py: Python<'_>, m: &Bound<'_, PyModule>) -> PyResult<()> {
    // generator functions
...
pub fn add_datalayer_submodule(py: Python<'_>, parent: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    py.import("sys")?
        .getattr("modules")?
        .set_item("chik_rs.datalayer", datalayer)?;
